...
<head>
    ...
    <title>Manage Schools – GiveToGrow Admin</title>
    ...
    <aside class="w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <nav class="flex-1 p-4">
            <ul class="space-y-2">
                ...
                <li>
                    <a href="manage_schools.php" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary/10 text-primary font-medium">
    ...
    <main class="flex-1 overflow-auto">
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-8 py-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-[#131514] dark:text-background-light">Manage Schools</h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-1">View and manage all registered schools</p>
                </div>
                <a href="add_school.php" class="flex items-center gap-2 px-6 py-3 bg-primary text-white rounded-lg font-bold hover:bg-opacity-90">
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Location</th>
                                <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Students</th>
                                <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Fundraising</th>
                                <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Needs</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <?php foreach ($schools as $school): 
                                $progress = ($school['fundraising_goal'] > 0) ? ($school['amount_raised'] / $school['fundraising_goal']) * 100 : 0;
                            ?>
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-900">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <img src="<?php echo htmlspecialchars($school['image_url']); ?>" alt="" class="w-12 h-12 rounded-lg object-cover mr-3"/>
                                        <div>
                                            <div class="text-sm font-medium text-[#131514] dark:text-background-light"><?php echo htmlspecialchars($school['school_name']); ?></div>
                                            <div class="text-xs text-gray-500">ID: <?php echo $school['school_id']; ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-700 dark:text-gray-300">
                                        GHS <?php echo number_format($school['amount_raised'], 0); ?> / <?php echo number_format($school['fundraising_goal'], 0); ?>
                                    </div>
                                    <div class="w-32 bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-2">
                                        <div class="bg-primary h-1.5 rounded-full" style="width: <?php echo min(100, $progress); ?>%"></div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-700 dark:text-gray-300"><?php echo $school['total_needs']; ?> total</div>
                                    <div class="text-xs text-green-600"><?php echo $school['active_needs']; ?> active</div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 text-xs font-medium rounded-full <?php 
                                        echo $school['status'] == 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 
                                            ($school['status'] == 'completed' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' : 
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex gap-2">
                                        ...
                                        <a href="add_need.php?school_id=<?php echo $school['school_id']; ?>" 
